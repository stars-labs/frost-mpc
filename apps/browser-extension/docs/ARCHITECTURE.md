...
- FROST-WASM host (loads `@frost-mpc/core-wasm` → `frostDkg`)
...
`packages/@frost-mpc/types/src/messages.ts` (shared workspace
...
**File:** `packages/@frost-mpc/types/src/messages.ts:53` (shared
...
**File:** `packages/@frost-mpc/types/src/messages.ts:190` — real
...
**File:** `packages/@frost-mpc/types/src/messages.ts` — two
...
**File:** `packages/@frost-mpc/types/src/messages.ts` — same
inner/wrapper split:
...
`packages/@frost-mpc/types/src/messages.ts:303`. Illustrative
...
Defined at `packages/@frost-mpc/types/src/messages.ts:262-296`
...
// Mesh Status Tracking (shared type, from @frost-mpc/types):
enum MeshStatusType {
    Incomplete,
    PartiallyReady,
    Ready
...
### Key Files
- `packages/@frost-mpc/types/src/messages.ts` — all message
  type definitions (shared workspace package — types aren't
  under the extension's own `src/`)
- `packages/@frost-mpc/types/src/appstate.ts` — application
